...
function directtex(str) coroutine.yield(str) end

function to_TeX_box(str)
    directtex("\\ExplSyntaxOn\\newbox\\kanbun_lua_box\\sbox\\kanbun_lua_box{" ..
                  str .. "}\\ExplSyntaxOff")
...
function main_loop()
    ...
    for i, l in ipairs(verb_table) do
        ...
        for i, c in ipairs(split_line) do last_index = i end
        ...
        for j, c in ipairs(split_line) do
            ...
            if last_bracket_index == 0 then
                ...
                if tex_kana_bool.mode == tex_true_bool.mode then
                    split_line[j] = brackets["okurigana"][1] .. c ..
                                        brackets["okurigana"][2]
                else
                    split_line[j] = "“⌊" .. c .. "⌋”"
                end
            end
    ...
    for i, l in ipairs(lines_chars_table) do
        ...
        line = string.gsub(line, brackets["okurigana"][2] ..
                               brackets["okurigana"][1], "")
        for k, v in pairs(brackets) do
            line = string.gsub(line, "”(%" .. v[1] .. ")", "%1")
            line = string.gsub(line, "(%" .. v[2] .. ")(“)", "%1”%2")
        end
        for c in line:gmatch(utf8.charpattern) do last = c end
        if has_value(right_brackets, last) then str = str .. '”' end
        ...
        local tmp_number_of_multikanji_braces = {
            utf8.char(61442), utf8.char(61443)
        }
        ...
        for j, u in ipairs(units) do
            ...
            for m, c in ipairs(split_unit) do
                if m == last_k then
                    split_unit[m] =
                        "“" .. tmp_number_of_multikanji_braces[1] ..
                            number_of_multikanji ..
                            tmp_number_of_multikanji_braces[2]
                end
            end
        ...
        for j, u in ipairs(units) do
            if u == "" then table.remove(units, j) end
        end
        local next_left_punct_ = ""
        for j, u in ipairs(units) do
            local right_okuri_ = u:match("%{(.-)%}") or ""
            local kanji_ = u:match("⌊(.-)⌋") or ""
            local right_furi_ = u:match("%((.-)%)") or ""
            local left_furi_ = u:match("‹(.-)›") or ""
            local left_okuri_ = u:match("«(.-)»") or ""
            local punct_ = u:match("⦉(.-)⦊") or ""
            --
            -- punct_ = punct_:gsub("―", "\\tateten")
            -- punct_ = punct_:gsub("—", "\\tateten")
            -- punct_ = punct_:gsub("〻", "\\ninojiten")
            --
            local kaeriten_ = u:match("%[(.-)%]") or ""
            local multikanji_ = u:match(tmp_number_of_multikanji_braces[1] ..
                                            "(.-)" ..
                                            tmp_number_of_multikanji_braces[2]) or
                                    0
            if punctuation_str:match(kanji_) then
                punct_ = kanji_
                kanji_ = ""
            end
            local left_punct_ = next_left_punct_
            next_left_punct_ = ""
            ...
            if u:match(tmp_number_of_multikanji_braces[1]) then
                local multiruby_raise_by_ = 0
                for trace_back_index = 1, multikanji_ - 1 do
                    multiruby_raise_by_ =
                        multiruby_raise_by_ +
                            to_TeX_box(units[j - trace_back_index]).width /
                            (kanbunzwtosp)
                end
                units[j - multikanji_ + 1] =
                    units[j - multikanji_ + 1]:gsub("\\kanjiunit",
                                                    "\\hbox{\\kanjiunit")
                units[j] = "\\kanjiunit{\\multifuriokuri[" ..
                               multiruby_raise_by_ .. "]{" .. right_furi_ ..
                               "}{" .. right_okuri_ .. "}}{" .. left_punct_ ..
                               "}{" .. kanji_ .. "}{" .. punct_ .. "}{" ..
                               kaeriten_ .. "}{\\multifuriokuri[" ..
                               multiruby_raise_by_ .. "]{" .. left_furi_ .. "}{" ..
                               left_okuri_ .. "}}}"
            else
                if kanji_ == "" then
                    units[j] = ""
                    ...
                    units[j] =
                        "\\kanjiunit{\\furiokuri{" .. right_furi_ .. "}{" ..
                            right_okuri_ .. "}}{" .. left_punct_ .. "}{" ..
                            kanji_ .. "}{" .. punct_ .. "}{" .. kaeriten_ ..
                            "}{\\furiokuri{" .. left_furi_ .. "}{" ..
                            left_okuri_ .. "}}"
                end
            end
        end
    ...
    -- ouput
    output = "{\\kanbunfont" .. table.concat(annotated_lines_table, "\\par") ..
                 "\\par}"
    ...
    directtex(
        "\\def\\printkanbunnoparcode{\\directlua{print('')print(nopar_output)}}")
...
function determine_ichire_kern()
    ...
    local ichirekern = tostring(approximate(curr_kern, prev_kern,
                                            ichire_untouched) / 100)

    directtex("\\setkanbun{ichirekern=" .. ichirekern ..
                  "}\\kanbun@msg{Automatically setting ichirekern to " ..
                  ichirekern .. ".}")
