...
\begin{ltxsyntax}
    ...
    \SaveVerb{newline}|\\|
    \en{The no-paragraph counterpart of \cmd{printkanbun}, which does not invoke \cmd{kanbunfont}, and uses \UseVerb{newline} instead of \cmd{par}.}
    \ja{\cmd{printkanbun} に対応する段落無しのバージョン。\cmd{kanbunfont}を使用せず、 \cmd{par} の代わりに \UseVerb{newline} を使用する。}
