...
\date{\en{\scshape 15 january 2022, v1.0}\ja{2022年1月15日, v1.0}}
...
\ja{更に重要なことには、 \LuaLaTeX{}で実行すれば、軽量マークアップの「漢文アノテーション」で容易く効率的に漢文訓読を組版することが可能となっている\footnote{JavaScriptとCSSによって実行するUntPhesocaの \href{https://phesoca.com/kanbun-html/}{漢文HTML} が参考となっていた。}。}
...
\begin{optionlist}
    ...
    \ja{漢字の書体のコマンドを設定する。\sty{luatexja-fontspec}パッケージが読み込まれている場合、 \UseVerb{fontcmd} に初期化されて、旧字体のグリフに変更する。}
    ...
    \ja{ルビ文字の書体のコマンドを設定する。\sty{luatexja-fontspec}パッケージが読み込まれている場合、 \UseVerb{rubyfontcmd} に初期化されて、ルビグリフに変更する。}

    \SaveVerb{unit}|\kanbun_zw|
    \optitem[]{unit}{\prm{\en{length}\ja{長さ}}}
    \en{Sets the base size (size of \emph{kanji}). Default: \UseVerb{unit}, which is initialised as \verb|1em|.}
    \ja{漢字の基本サイズを設定する。初期値： \UseVerb{unit}、つまり \verb|1em|。}
    ...
    \ja{ルビ文字のサイズに対する送り仮名\ruby[pre=0.5,post=1]{進入量}{イントルージョン}（送り仮名を上に突かせて漢字に掛ける長さ）の割合を設定する。初期値： \verb|1|。}
    ...
    \ja{アキ組を使うかベタ組を使うかを設定する。\opt{kumi=} を書かず、単に \opt{aki} か \opt{beta} かと書いても大丈夫。}
\end{optionlist}
...
\ja{漢文アノテーションの機能を使わない場合、任意の CJK 支援のあるエンジンで漢文を組版できる。例えば\XeLaTeX{} を \sty{xeCJK} パッケージと一緒に使うか、 \upLaTeX{} を \sty{utarticle} クラス或いは他の適宜なクラスと一緒に使うか、どちらも可。}

\begin{ltxsyntax}
    \cmditem{kanjiunit}\verb|{ |\cmd{furiokuri}\mprm{\en{right furigana}\ja{右の振り仮名}}\mprm{\en{right okurigana}\ja{右の送り仮名}}\verb| }|\vspace{-5pt}\\
    \mprm{\en{left (top) punctuation (e.g.~`「')}\ja{左（上）の句読点（“「”など）}}\\
    \mprm{\en{kanji}\ja{漢字}}\\
    \mprm{\en{other punctuation}\ja{他の句読点}}\\
    \mprm{\en{kaeriten}\ja{返り点}}\\
    \verb|{ |\cmd{furiokuri}\mprm{\en{left furigana}\ja{左の振り仮名}}\mprm{\en{left okurigana}\ja{左の送り仮名}}\verb| }|
    ...
    \cmditem{multifuriokuri}\oprm{\en{length by which ruby is raised}\ja{ルビ文字を上げる長さ}}\mprm{\en{furigana}\ja{振り仮名}}\mprm{\en{okurigana}\ja{送り仮名}}
\end{ltxsyntax}
...
\begin{itemize}
    \item \verb|( )|: \en{\emph{furigana} (}振り仮名\en{)}
    \item \verb|{ }|: \en{\emph{okurigana} (}送り仮名\en{) (these brackets can be omitted)}\ja{（この括弧を省略しても大丈夫）}
    \item \verb|‹ ›|: \en{\emph{furigana} (振り仮名) of \emph{saidoku-moji} (再読文字)}\ja{再読文字の振り仮名}
    \item \verb|« »|: \en{\emph{okurigana} (送り仮名) of \emph{saidoku-moji} (再読文字)}\ja{再読文字の送り仮名}
    \item \verb|[ ]|: \en{\emph{kaeriten} (}返り点\en{)}
    \item \verb|‘ ’|: \en{multiple \emph{kanji}, potentially with \emph{tateten} inserted, as a ruby base; group ruby (グループルビ)}\ja{グループルビのベース。中に\ruby{竪|点}{たて|てん}を入れても可}
...
\en{You can always save \cmd{printkanbun} to a macro and start a new annotated \emph{kanbun} block, as in the following example.}
\ja{次の実例のとおり、いつでも \cmd{printkanbun} をあるマクロに保存し、新しい漢文段落を書き込み始められる。}
\begin{example}
% example text from https://phesoca.com/kanbun-html/
\documentclass{ltjtarticle}
